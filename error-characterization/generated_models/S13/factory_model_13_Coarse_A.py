...
# Class representing a single Stage (Machine + Buffer)
class Stage(Node):
    def __init__(self, env, id, stage_idx, delay, capacity):
        super().__init__(env, id)
        # Creating Machine with specific ID from assumptions
        self.m = Machine(env, id=f"Stage_{stage_idx}_M1", processing_delay=delay)
        self.add_child_node(self.m)
        
        # Creating Buffer with specific ID from assumptions
        self.b = Buffer(env, id=f"Stage_{stage_idx}_B1", capacity=capacity)
        self.add_child_edge(self.b)

# Class representing the Linear Sequence (Row)
class LinearSequence(Node):
    def __init__(self, env, id):
        ...
        # Creating 3 Stages
        # Delays: 4, 3, 2. Capacities: 2, 3, 4.
        delays = [4.0, 3.0, 2.0]
        capacities = [2, 3, 4]
        self.stages = []
        for i in range(3):
            stage = Stage(env, id=f"Stage_{i+1}", stage_idx=i+1, delay=delays[i], capacity=capacities[i])
            self.stages.append(stage)
        self.add_child_node(self.stages)
        
        # Buffer connecting Source to first Stage (Default capacity 2)
        self.b_src = Buffer(env, id="B_src_1", capacity=2)
        self.add_child_edge(self.b_src)
        
        # Connecting Source to Stage 1 Machine
        self.b_src.connect(self.source, self.stages[0].m)
        
        # Connecting Stages internally
        # Stage 1 Buffer connects Stage 1 Machine to Stage 2 Machine
        self.stages[0].b.connect(self.stages[0].m, self.stages[1].m)
        
        # Stage 2 Buffer connects Stage 2 Machine to Stage 3 Machine
...
# Class representing the overall System Model
class SystemModel(Node):
    def __init__(self, env, id):
        super().__init__(env, id)
        
        # Creating 4 parallel LinearSequences (Rows)
        self.rows = [LinearSequence(env, id=f"Row_{i+1}") for i in range(4)]
        self.add_child_node(self.rows)
        
        # Exposing sources and sinks from rows
        self.sources = [row.source for row in self.rows]
        self.sinks = [row.sink for row in self.rows]
        
        # Cross Buffer 1: Row 1 (index 0) M1 to Row 2 (index 1) M1
        ...
        # Cross Buffer 2: Row 2 (index 1) M3 to Row 4 (index 3) M3
