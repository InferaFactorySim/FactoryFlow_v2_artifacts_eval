...
class Subsystem(Node):
    def __init__(self, env, id):
        ...
        # Creating buffer to connect machines in series
        self.e = Buffer(env, id="e[0]")
        self.add_child_edge(self.e)
        
        # Connecting M[0] -> Buffer -> M[1]
        self.e.connect(self.M[0], self.M[1])
...
class SystemModel(Node):
    def __init__(self, env, id):
        ...
        self.subsystems = [Subsystem(env, id=f"Cell_X_{i+1}") for i in range(3)]
        self.add_child_node(self.subsystems)
        
        # Creating buffers for sequential connections
        # Need 4 buffers: Source->Sub1, Sub1->Sub2, Sub2->Sub3, Sub3->Sink
        self.edges = [Buffer(env, id=f"edge[{i}]") for i in range(4)]
        self.add_child_edge(self.edges)
        
        # Connecting Source to first subsystem (M[0])
        self.edges[0].connect(self.source, self.subsystems[0].M[0])
        
        # Connecting subsystems in series
        for i in range(2):
            # Connect output of current subsystem (M[1]) to input of next subsystem (M[0])
            self.edges[i+1].connect(self.subsystems[i].M[1], self.subsystems[i+1].M[0])
            
        # Connecting last subsystem (M[1]) to Sink
