...
class Subsystem_1(Node):
    def __init__(self, env, id):
        ...
        # Creating edges
        # Buffer from src to M1 (ID B1)
        self.B1 = Buffer(env, id="B1", capacity=2)
        # Buffer from M1 to M2 (ID B2, capacity 5)
        self.B2 = Buffer(env, id="B2", capacity=5)
        # Buffer from M2 to Machine_Check (ID Buffer_Check)
        self.Buffer_Check = Buffer(env, id="Buffer_Check", capacity=2)
        # Buffer from Machine_Check to M1 (Rework, ID B3, capacity 1)
...
class SystemModel(Node):
    def __init__(self, env, id):
        ...
        # Creating edges between Subsystem_1 instances and Subsystem_2
        self.edges = [Buffer(env, id=f"edge[{i}]", capacity=2) for i in range(3)]
        self.add_child_edge(self.edges)
        
        # Connecting Machine_Check of each Subsystem_1 to Mac_M1 of Subsystem_2
        for i in range(3):
            self.edges[i].connect(self.sub1[i].Machine_Check, self.sub2.Mac_M1)
