...
2. Added a Source node ('SRC') to feed the first machine (M1) and a Sink node ('SINK') to receive output from the last machine (M4) to complete the system flow.Machine IDs are M1, M2, M3, M4, . This forms a class "cluster"
...
class SystemModel(Node):
    def __init__(self, env, id):
        ...
        # Defining connections as (source_node, destination_node) tuples
        connections = []
        
        # Source to M1
        connections.append((self.source, self.machines[0]))
        
        # Linear sequence M1 -> M2 -> M3 -> M4
        for i in range(3):
            connections.append((self.machines[i], self.machines[i+1]))
            
        # M4 to Sink
        connections.append((self.machines[3], self.sink))
        
        # Feedback M2 -> M1
        connections.append((self.machines[1], self.machines[0]))
        
        # Feedback M4 -> M1
        connections.append((self.machines[3], self.machines[0]))

        # Creating buffers and connecting nodes
        self.buffers = []
        for src_node, dst_node in connections:
            buf_id = f"B_{src_node.id}_{dst_node.id}"
