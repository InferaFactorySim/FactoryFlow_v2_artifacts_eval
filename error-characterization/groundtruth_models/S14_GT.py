"""A production line begins with a Source named Src generating items every 5 seconds. 
Src connects to first Machine via B_src_1 The line consists of 3 sequential stages. 
Each stage Stage_1 through Stage_3 is modeled as a subsystem containing a Machine with 
a processing delay of 4.0 seconds, followed by a Buffer with capacity 2. The output of 
Stage_3 is connected to a Sink named Sink. There are 4 such sequences in parallel. Name 
machines in stage_i as Stage_i as Stage_i_M1,, and Buffers as Stage_i_B1,for i from 1 
to 3. All machines have a processing_Delay of 2 and buffers have a capacity of 2. 
There is a connection between the M1 in r1 and M1 in row2 and a connection between M3 
in row2 and M3 in row4. In every row and the 2nd and 3rd machines have 3 buffers in 
between them.Name cross conecting buffer as Cross_Buffer_1, Cross_Buffer_2, etc.
Row index starts from 1. 
And extra buffers in multiedge as B_2_3_1, B_2_3_2"""
...
path = fig.render(filename="S14_GT", directory=".", format="png", cleanup=True)
TOP.validate()
TOP.show_hierarchy()
