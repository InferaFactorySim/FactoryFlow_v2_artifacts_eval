...
SRC is followed by a buffer ID=B_src_1. The line consists of 3 sequential stages. 
Each stage Stage_1 through Stage_3 is modeled as a subsystem containing a Machine with 
a processing delay of 2.0 seconds, followed by a Buffer with capacity 2. The output of 
Stage_3 is connected to a Sink named SINK. There are 4 such sequences in parallel. 
...
from 1 to 3. 
 ...
 in row2 and M3 in row4. 
Name cross conecting buffer as Cross_Buffer_1, Cross_Buffer_2, etc."""
...
path = fig.render(filename="S12_GT", directory=".", format="png", cleanup=True)
TOP.validate()
TOP.show_hierarchy()
